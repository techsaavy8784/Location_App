...
import { LocationMock } from "../utils/LocationMock";
...
const LocationMap = ({ range }) => {
  ...
  const fetchNearbyMarkers = (lat, lng) => {
    const inDistance = getMarkersWithinDistance(center, LocationMock, range);
    setNearbyMarkers(inDistance);
    const nearlydistance = getNearestMarker(center, LocationMock);
